...
export const sendRequest = async (
  ...
  try {
    ...
    return createResponseObject(response, url)
  } catch (error) {
    console.error('请求错误:', error)
    if (axios.isCancel(error)) {
      throw new Error(`请求被取消: ${error instanceof Error ? error.message : String(error)}`)
    }

    const axiosError = error as AxiosError

    if (axiosError.response) {
      return createResponseObject(axiosError.response, url, false)
    }
...
/**
 * 创建一个与fetch API兼容的Response对象
 */
function createResponseObject(axiosResponse: AxiosResponse, url: string, ok = true): Response {
  const responseHeaders = new Headers()
  const axiosHeaders = axiosResponse.headers
