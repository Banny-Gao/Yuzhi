...
import { ResponseStatus, StatusGroups, isRetryable } from './status'
 ...
 */
export function withRetry<T>(requestFn: () => Promise<T>, retryConfig: RetryConfig = DEFAULT_RETRY_CONFIG): Promise<T> {
 ...
 * @param options 超时选项
 */
export function withTimeout<T>(requestFn: (() => Promise<T>) | CancelablePromise<T>, options: TimeoutOptions): Promise<T> {
 ...
 * @param options 缓存选项
 */
export function withCache<T>(requestFn: (config?: AxiosRequestConfig) => Promise<T>, options: CacheOptions, requestConfig?: AxiosRequestConfig): Promise<T> {
